...
export function AnswerBox({text, ok, big, block}: AnswerBoxProps) {
    return (
        <div className={`${block ? 'flex' : 'inline-flex'} items-${block ? 'start' : 'center'} gap-2 rounded-xl border-2 ${ok ? 'border-lime-200' : 'border-red-600 bg-red-600/10'} ${big ? 'px-4 py-2.5' : 'px-3 py-2'}`}>
            ...
            <span className={`font-black flex-shrink-0 ${big ? 'text-base' : 'text-sm'} ${ok ? 'text-cyan-800' : 'text-red-600'}`}>
                {ok ? <MaskedSvgIcon src="/images/tick.svg" alt="Correct" size="w-6 h-6" color="bg-lime-200" /> : '✕'}
            </span>
        </div>
    );
}
