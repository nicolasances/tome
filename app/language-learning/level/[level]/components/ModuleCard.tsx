    ...
    return (
        ...
        >
            <div className="flex items-start justify-between">
                ...
                <span className={`text-3xl font-bold leading-none ${isCurrent || isActive ? 'text-lime-200' : 'text-black/30'}`}>{num}</span>

                {(isCurrent || isActive) && <RoundButton svgIconPath={{ src: '/images/point-right.svg', alt: 'Open' }} type="primary" onClick={() => {}} size="s" />}
            ...
            <div>
                <span className={`text-base font-bold leading-tight ${isCurrent || isActive ? 'text-lime-200' : 'text-black/70'}`}>
                    {module.title}
                </span>
                {isCurrent || isActive ? (
