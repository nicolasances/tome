...
export function PracticeResult({ type, text }: PracticeResultProps) {
    ...
    return (
        <div className='flex flex-col items-stretch text-left'>
            <div className={`flex text-base rounded-md items-center ${type != 'correct' ? 'px-4' : 'px-2'} py-2 border-2 ${type === 'correct' ? 'border-green-800 text-green-800' : type === 'incorrect' ? 'border-red-800 text-red-800' : 'border-cyan-400 text-cyan-200'}`}>
                <div className="">
                ...
                <div className={`flex-1 flex flex-col items-start justify-center ${type != 'correct' ? 'pl-4' : 'pl-1'} border-l-4 border-[var(--background)] self-stretch -my-2 py-2`}>
                    <div className="text-lg">{text || <em className="text-muted-foreground">empty</em>}</div>
                </div>
            </div>
        </div>
    );
