...
| Home dashboard | Continue CTA | Dark rounded card with a lime circular arrow, kicker "Continue · A1·01", and the current module title. | Tapping navigates to the current module (Module overview, or its current step). When no module is in progress, it is **replaced** by one of two states: (a) if all curated modules at the level are complete **and** the user is eligible for the Level Test → the **"Level Test unlocked"** CTA (see next row); (b) otherwise a passive empty state. |
