...
| Practice complete · **Round complete** | Two-ring coverage (`AnimRing`, two concentric circles + `SparkBurst`) | **Outer ring — rung coverage**: completed rungs out of 3 (`rungsCompletedBefore/3 → rungsCompletedAfter/3`), with the rung name (e.g. "Rung 2 · Cued production") beneath. **Inner ring — vocabulary coverage**: distinct items covered ÷ total, unchanged from the single-ring design it replaces. A restrained lime spark burst plays on mount. | Both rings **sweep from their before value to their after value** on mount, so the round's contribution is felt. **Both are monotonic — neither ever resets** (design doc OQ-03): the outer ring only climbs as rungs complete; the inner ring climbs 0→100% during rung 1 and then sits near 100% through rungs 2–3 (it does not track rung-specific coverage). |
...
| Practice complete · **Coverage milestone** (ladder complete) | Milestone medallion | Lime tick medallion with a spark burst; headline "All words covered" + subline naming the module. | Marks the bigger moment: **rung 3 is complete** — every vocabulary item and grammar concept has now been produced from meaning alone (`ladderCompleted: true`), not just recognised once. |
| Practice complete · **Coverage milestone** | Coverage bar (`AnimBar`) | Module-coverage bar with an "N / N words" count (vocabulary coverage, same figure as the inner ring elsewhere). | **Fills to 100%** on mount (ghost shows the pre-round level); the count reads full (e.g. "8 / 8 words"). |
...
- **Two-ring recap animates old→new**: on the Round complete state, the outer ring animates `rungsCompletedBefore/3 → rungsCompletedAfter/3` — both returned directly by `/complete`. The inner ring animates the module's vocabulary coverage from its value **before** this session (fetched from `GET /me/progress` when the session started, the same pre-round snapshot the client already carries today as `prevCovered`) to `vocabularyCoverage.coveredCount / vocabularyCoverage.totalCount` **after** this session, from the `/complete` response — the response carries only the after-state, so the before-state continues to come from the pre-session fetch. Both rings are monotonic — neither is ever reset by a rung-phase transition (design doc OQ-03), which is what keeps the per-rung reset invisible to the user. The Coverage milestone's bar animates the same vocabulary-coverage figure to 100%. The Coverage milestone state is shown iff this round brought the ladder to completion (`ladderCompleted: true`, i.e. rung 3 finished).
...
| Practice complete · Coverage milestone (test-unlock card) | `GET /me/progress` (owned by `03-module-overview`) | Read-back of `testUnlocksAt` to show "Module test unlocks in 4h" on the milestone card, since `/complete` does not return it. Also the source of the pre-round vocabulary-coverage snapshot the two-ring recap needs for its "before" value (fetched when the practice session starts, not at completion — see §4). |

**Missing**

| Component or Screen | Missing API endpoint / field |
| ------------------- | ---------------------------- |
| Practice overview between-round display (`03-module-overview`) | `GET /me/progress` does not return per-rung state (`currentRung` / rungs completed), only module-wide `vocabularyItemsPracticedCount`. The overview's coverage bar can't show rung progress yet — see `03-module-overview` §7 open question. |
...
| 19 | When `ladderCompleted` is `false`, the **Round complete** state shows the two-ring recap — outer ring sweeping `rungsCompletedBefore/3 → rungsCompletedAfter/3`, inner ring sweeping the previous vocabulary-coverage % to the new one — the round stats (answered / mastered / accuracy), and the CTAs "Practice another round" + "Back to module". Neither ring ever resets across a rung transition. | New wireframe (A), extended for the practice ladder (issue #321). |
...
| ~~6~~ | ~~Does `POST .../complete` return the **before/after coverage figures** (or counts) the recap needs for the old→new ring/bar sweep, or must the client fetch them separately?~~ | **Resolved** (issue #321) — `/complete` now returns `rungsCompletedBefore/After`, `rungCoverageBefore/After`, and `vocabularyCoverage` (after-state only; the before-state for the inner ring still comes from the pre-session `GET /me/progress` fetch, as it did for the single-ring design). See §5.1. |
