...
introduces the module's vocabulary and grammar through exercises. Step 2 runs as
**three sequential rung phases** of increasing difficulty — recognition → cued
production → free production (the "practice ladder", issue #321; design:
`docs/idea/language-learning/2026-07-29-practice-ladder.md`). Step 2 is **not a
single session** — the user runs `practiceSessionSize`-sized sessions back-to-back
within a rung phase until **every vocabulary item and every grammar concept in the
module has been served an exercise at that rung**, then the module advances to the
next rung; practice completes when rung 3 completes (§3.1.1). Each exercise
...
| Practice | Exercise body — Translation (active) | English prompt with an inline, auto-growing/wrapping text input (`AnswerAreaInput`) directly beneath it. | Free production; typed directly under the prompt, submitted via a standalone send `RoundButton`. |
| Practice | Free-text inputs (`AnswerLineInput`, `AnswerAreaInput`, Conjugation Drill, Error Correction) | Every typed-answer exercise input. | On desktop, **Cmd+A** inserts "æ" at the cursor (replacing any selection) in whichever of these is focused, instead of the browser's native select-all (issue #325; keyboard logic in `utils/useKeyboardShortcuts`). |
...
| Practice | Send button (standalone `RoundButton`) | Right-aligned send `RoundButton`, no full-width footer or text input alongside it. | Used by Fill in the Blank, Conjugation Drill, and Translation — all three type inline in the exercise body (`AnswerLineInput` / `AnswerAreaInput`) rather than in a bottom-anchored footer input, avoiding the iOS Safari issue where opening the keyboard for a bottom-anchored input scrolls the prompt out of view (see issues #315, #317). The former `SendFooter` component (text input + send button in one bottom bar) has been removed. |
...
| Practice | Sheet action buttons (`SheetBtn`) | Light outline buttons shown inside the wrong-answer `ResultSheet`. | "Explain my mistake" (magic icon) appears on all wrong-answer states and is still a stub. "Check with AI" (teacher icon) appears only on Translation wrong-answer states and is **wired**: tapping it replaces the `ResultSheet` with the `AIVerifyTray` (see below). On desktop, **Cmd+.** triggers it too, no-op when the button isn't shown (issue #325; keyboard logic in `utils/useKeyboardShortcuts`). |
...
| Practice complete · **Round complete** | Two-ring coverage (`AnimRing`, two concentric circles + `SparkBurst`) | **Outer ring — rung coverage**: completed rungs out of 3 (`rungsCompletedBefore/3 → rungsCompletedAfter/3`), with the rung name (e.g. "Rung 2 · Cued production") beneath. **Inner ring — current-rung coverage**: items (vocabulary + grammar concepts pooled) covered ÷ total **at the rung just practiced** (`rungCoverageBefore/After`) — supersedes the module-wide vocabulary-coverage figure it originally replaced. A restrained lime spark burst plays on mount. | The outer ring **sweeps from its before value to its after value** on mount, so the round's contribution is felt. The outer ring is **monotonic — never resets** (design doc OQ-03). The inner ring is **intentionally NOT monotonic across rounds** (issue #323, supersedes OQ-03 for the inner ring only): within one recap it sweeps its own before→after (same rung, so it doesn't decrease), but the first recap shown for a new rung starts that rung's coverage low again — a deliberate reset that makes the rung transition visible, rather than the old design's saturating-near-100%-forever inner ring. |
| Practice complete · **Round complete** | Headline + subline | Large performance-driven headline ("Round complete", or "Rung complete!" when `rungCompleted` is true) and an encouraging subline. | Tone adapts to how the round went (e.g. strong vs. needs-work) and to whether a rung just completed. |
...
| Practice complete · **Round complete** | Primary CTA — "Practice another round" (`PrimaryBtn`, lime) | Full-width lime button. | **Starts a new practice session** (loops back into the practice run — the next session is simply the first of the new rung if one just completed; no gate in between, per design doc OQ-02). |
...
| Practice complete · **Coverage milestone** (ladder complete) | Milestone medallion | Lime tick medallion with a spark burst; headline "All items covered" + subline naming the module. | Marks the bigger moment: **rung 3 is complete** — every vocabulary item and grammar concept has now been produced from meaning alone (`ladderCompleted: true`), not just recognised once. |
| Practice complete · **Coverage milestone** | Coverage bar (`AnimBar`) | Module-coverage bar with an "N / N items" count (vocabulary + grammar concepts pooled — rung 3's coverage, which is 100% by definition at this state). | **Fills to 100%** on mount (ghost shows the pre-round level); the count reads full (e.g. "10 / 10 items"). |
| Practice complete · **Coverage milestone** | Test-unlock card | Dark card with a lock tag: "Module test unlocks in 4h" + a spaced-repetition explanation. | Communicates that Step 2 (the whole ladder) is complete and the test enters its `testUnlockDelayHours` cool-down. The countdown/lock itself is owned by the Module overview / test (`03`/`06`); this card is the in-context announcement. |
...
| Practice complete · **Coverage milestone** | Secondary CTA — "Keep practising" (`GhostBtn`) | Full-width ghost button. | Starts a new practice session even though the ladder is complete (drill while the test cools down). |
...
- **End of round**: after the full exercise list is completed, missed exercises are retried until all are answered correctly, then `POST .../complete` is called. The save happens in the background (no blocking overlay) and the client transitions to the **Practice complete screen** — it does **not** auto-route. Which state shows is selected by `ladderCompleted` in the `/complete` response: `false` → **Round complete**; `true` → **Coverage milestone**. (`step2Complete` is kept on the response as an alias of `ladderCompleted` for older clients — this feature reads `ladderCompleted`.)
- **User-driven loop**: from the **Round complete** state the user chooses "Practice another round" (start a new session via `POST /practiceSessions`) or "Back to module" (navigate to `03-module-overview`). The loop is no longer an automatic hand-off — the recap is the decision point. There is **no dedicated "rung complete" screen and no hard stop between rungs** (design doc OQ-02): when a round happens to complete a rung, the user still sees the ordinary **Round complete** state, just with the outer ring having ticked over and the headline reflecting it; the next session is simply the first of the new rung. Between-round coverage progress also remains visible on the overview (owned by `03-module-overview`, though it does not yet expose rung state — see its §7 open question).
- **End of Step 2 (ladder complete)**: when `/complete` returns `ladderCompleted: true` (rung 3 done) the **Coverage milestone** state is shown. The test-unlock countdown begins server-side (owned by `03`/`06`); the milestone card announces it in context. From here "Back to module" returns to the overview and "Keep practising" starts another session.
- **Recap data**: round stats (answered, mastered = first-try-correct, accuracy) are derived from the round's local `answers` log. Rung/coverage figures (before/after rung counts, before/after vocabulary coverage) and the milestone's unlock timing come from the backend — see §5.1 for the exact contract.
...
- **Two-ring recap animates old→new**: on the Round complete state, the outer ring animates `rungsCompletedBefore/3 → rungsCompletedAfter/3` — both returned directly by `/complete`. The inner ring animates `rungCoverageBefore.coveredCount/totalCount → rungCoverageAfter.coveredCount/totalCount` — **both before and after are returned directly by `/complete`** (issue #323; no separate pre-session fetch is needed for this ring anymore, unlike the module-wide figure it replaced). The outer ring is monotonic — never reset by a rung-phase transition (design doc OQ-03). The inner ring is **not** monotonic across rounds (OQ-03 superseded for this ring by issue #323): because `rungCoverageBefore/After` describe the rung just practiced, the first recap shown after a rung transition starts that rung's coverage low again — the reset is visible round to round, communicating that a new, harder rung has begun. The Coverage milestone's bar animates the module-wide item-coverage figure to 100%. The Coverage milestone state is shown iff this round brought the ladder to completion (`ladderCompleted: true`, i.e. rung 3 finished).
...
| 8 | `POST .../complete` response includes `ladderCompleted: boolean`, `rungCompleted: boolean`, before/after rung counts, and vocabulary coverage — the full contract the recap renders (see §5.1). The client uses `ladderCompleted` to pick the Practice-complete state and the other fields to drive the two rings/bar/counts. | Drives the user-driven loop and the recap: the client never derives rung or coverage state itself. |
...
| Continue button (end of round) → Practice complete screen | `POST /users/:userId/practiceSessions/:sessionId/complete` | Marks the session complete. The server updates mastery (SRS) for all answered exercises, records rung coverage, and — when every item is covered at the current rung — completes that rung and advances the module (backend F10). Response:<br>`{ currentRung, previousRung, rungCompleted, ladderCompleted, rungsCompletedBefore, rungsCompletedAfter, rungCoverageBefore: { rung, coveredCount, totalCount }, rungCoverageAfter: { rung, coveredCount, totalCount }, vocabularyCoverage: { coveredCount, totalCount }, step2Complete, unseenVocabCount }`.<br>`step2Complete` and `unseenVocabCount` are kept as legacy aliases of `ladderCompleted` / `vocabularyCoverage.totalCount − coveredCount` for older clients; this feature reads the named fields. The client transitions to the Practice complete screen (state chosen by `ladderCompleted`); it does not auto-route. |
| Practice complete · Coverage milestone (test-unlock card) | `GET /me/progress` (owned by `03-module-overview`) | Read-back of `testUnlocksAt` to show "Module test unlocks in 4h" on the milestone card, since `/complete` does not return it. The two-ring recap's inner-ring before/after values no longer need a separate pre-round fetch — both come directly from `/complete`'s `rungCoverageBefore`/`rungCoverageAfter` (issue #323; see §4). |
...
| 17 | When `POST .../complete` returns `ladderCompleted: true`, the **Coverage milestone** state is shown instead of **Round complete**; the client does not auto-navigate to the module overview — the user chooses "Back to module" or "Keep practising". | §3.1.1. |
...
| 19 | When `ladderCompleted` is `false`, the **Round complete** state shows the two-ring recap — outer ring sweeping `rungsCompletedBefore/3 → rungsCompletedAfter/3`, inner ring sweeping the current rung's coverage % before → after this session — the round stats (answered / mastered / accuracy), and the CTAs "Practice another round" + "Back to module". The outer ring never resets across a rung transition; the inner ring intentionally does, round to round, once a new rung begins (issue #323). | New wireframe (A), extended for the practice ladder (issue #321); inner ring reworked for issue #323. |
| 20 | When `ladderCompleted` is `true`, the **Coverage milestone** state shows the coverage bar filling to 100% with "N / N words", the "Module test unlocks in …" card, and the CTAs "Back to module" + "Keep practising". | New wireframe (C). |
...
| ~~6~~ | ~~Does `POST .../complete` return the **before/after coverage figures** (or counts) the recap needs for the old→new ring/bar sweep, or must the client fetch them separately?~~ | **Resolved** (issue #321, and issue #323 for the inner ring specifically) — `/complete` returns `rungsCompletedBefore/After`, `rungCoverageBefore/After`, and `vocabularyCoverage`. The inner ring now sources both its before and after values from `rungCoverageBefore/After` directly — no separate pre-session `GET /me/progress` fetch is needed for it anymore. See §5.1. |
| 7 | Where does the milestone's "**Module test unlocks in 4h**" timing come from at completion — the `/complete` response or a follow-up `GET /me/progress`? | Still open — `/complete` does not carry `testUnlocksAt`; the milestone card reads it from the pre-session `GET /me/progress` fetch. A second call right after completion would give a fresher value but isn't currently made. |
