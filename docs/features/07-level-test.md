...
| OQ-2 | "Check with AI" (AI answer verification of a wrong Translation) is **not wired** for the Level Test — the verify delegate 404s on a level-test attempt id. Do we make the small backend change to wire it, or ship it as a stub in v1? | **Resolved for v1: stub.** The in-test `ResultSheet` always renders with `aiVerify={false}` for the Level Test, so "Check with AI" never appears on a wrong Translation (matching "Explain my mistake"'s existing stub). Wiring the backend delegate is tracked separately in issue [#310](https://github.com/nicolasances/tome/issues/310); when it ships, flipping that one prop is the only frontend change needed. |
