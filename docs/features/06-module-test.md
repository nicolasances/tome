...
| In-test | Result sheet (`ResultSheet`) | The same bottom sheet as practice: after every answer, slides up with **correct** (green `Verdict` + "Correct!") or **wrong** (red `Verdict` + "Not quite", correct answer revealed, `SheetBtn` actions) state. | Immediate per-answer feedback (see §4 and OQ-1). `ContinueBtn` advances to the next question. "Explain my mistake" on all wrong states is a stub; **"Check with AI" on Translation wrong is wired** — it opens the same `AIVerifyTray` as practice (F13), reusing the practice rendering. On desktop, **Cmd+.**/**Cmd+A** shortcuts apply here too (issue #325), reusing `05-practice-session`'s `ResultSheet`/free-text inputs as-is. |
