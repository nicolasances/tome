...
For `translation_active` exercises, the AI generates 3–5 accepted alternative answers alongside the canonical answer. These cover valid paraphrases and minor lexical variation (e.g. *Jeg vil have* vs *Jeg vil gerne have*). Single-word translation exercises typically have no alternatives. Answer matching is done after normalization (lowercase, punctuation stripped) against the full accepted answer set — no AI call is made at answer time.
