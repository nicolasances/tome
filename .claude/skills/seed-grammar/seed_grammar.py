#!/usr/bin/env python3
"""Seed grammar concepts for a Tome module via the batch API.
...
Usage:
    python3 seed_grammar.py <module-id> [--env dev|prod]
...
def _load_grammar(module_id: str) -> list:
    grammar_path = Path("content") / module_id / f"{module_id}-grammar.json"
...
def _call_batch_api(items: list, base_url: str, token: str) -> dict:
    url = f"{base_url}/grammarConcepts/batch"
...
def _parse_args() -> tuple[str, str]:
    ...
    if not args or args[0].startswith("-"):
        print("Usage: python3 seed_grammar.py <module-id> [--env dev|prod]")
        print("  e.g.: python3 seed_grammar.py A1-01 --env prod")
        sys.exit(1)
